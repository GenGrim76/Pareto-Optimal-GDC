...
public class Lemire_Fk_BIC_Decompr_exp {

    private Lemire_Fk_BIC_Decompr_exp() {
    }
    ...
    protected static void execute(String path, String Fk_To_decompr_in, String scenario, String approx_Suffix, int debug) throws InterruptedException, IOException {
        ...
        String Fk_decompr_EXT = ".txt.bic.d";
        String nfo_EXT = ".nfo";
        ...
        try {
            ...
            if (scenario.compareTo("a") == 0) {
                ...
                if (debug==1) System.out.println("BIC DECOMPR" + "\t" + "Read .nfo file from: " + path + Fk_To_decompr_in + FS_SEPARATOR + Fk_To_decompr_in + approx_Suffix + nfo_EXT);
            }
            else {
                ...
                if (debug==1) System.out.println("BIC DECOMPR" + "\t" + "Read .nfo file from: " + path + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + FS_SEPARATOR + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + approx_Suffix + nfo_EXT);
            }
        ...
        //FIXME: Read Fragment Compressed Data Sequence
        int i = 0;
        while (i < num_fragments) {
            ...
            // opening in reading fragment frequencies file
            ...
            try {
                ...
                if (scenario.compareTo("a") == 0) {
                    ...
                    if (debug==1) System.out.println("\nBIC DECOMPR - Read compressed frequencies from: " + path + Fk_To_decompr_in + FS_SEPARATOR + Fk_To_decompr_in + approx_Suffix + "_" + i + Fk_compr_EXT);
                }
                else {
                    // delete "-Gk" suffix from Fk_To_compr_in for reading .nfo file !!!.
                    ...
                    if (debug==1) System.out.println("\nBIC DECOMPR - Read compressed frequencies from: " + path + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + FS_SEPARATOR + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + approx_Suffix + "-Gk_" + i + Fk_compr_EXT);
                }
            ...
            // count index variable frequencies encountered
            int countFreq = 0;

            while ((l = Fk_compr_in.readLine()) != null) {
                // update counter of frequencies encountered
                countFreq++;
            }

            // Closing Fk_in input file
            Fk_compr_in.close();
            ...
            try {
                ...
                if (scenario.compareTo("a") == 0) {
                    Lemire_BIC_out = new PrintWriter(new FileWriter(path + Fk_To_decompr_in + FS_SEPARATOR + Fk_To_decompr_in + approx_Suffix + "_" + i + Fk_decompr_EXT));
                    if (debug==1) System.out.println("BIC DECOMPR" + "\t" + "Write data to: " + path + Fk_To_decompr_in + FS_SEPARATOR + Fk_To_decompr_in + approx_Suffix + "_" + i + Fk_decompr_EXT);
                }
                else {
                    // delete "-Gk" suffix from Fk_To_compr_in path for reading .nfo file !!!.
                    Lemire_BIC_out = new PrintWriter(new FileWriter(path + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + FS_SEPARATOR + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + approx_Suffix + "-Gk_" + i + Fk_decompr_EXT));
                    if (debug==1) System.out.println("BIC DECOMPR" + "\t" + "Write data to: " + path + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + FS_SEPARATOR + Fk_To_decompr_in.substring(0, Fk_To_decompr_in.length() -3) + approx_Suffix + "-Gk_" + i + Fk_decompr_EXT);
                }
            ...
            // Storing recovered data into Lemire_OptPFOR_out output file
            for (int x = 0; x < recovered.length; x++) {
                Lemire_BIC_out.println(recovered[x]);
            }

            // Closing  Lemire_OptPFOR_out output file
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        Lemire_Fk_BIC_Decompr_exp.execute("/home/user/IdeaProjects/TradeOff/exp-fragments/", "Staph-k4-DSK-verbatim", "b", "", 1);
    }
}
