...
public class Main_Launch_Exp {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        // Experiment Folder
        // for example: /home/nome-utente/IdeaProjects/TradeOff/experiments/StaphAU
        ...
        for (int k : k_values) {
            ...
            // FIXME: Extract (Dk,Fk) dictionary with KMC from dataset
            ...
            // FIXME: Canonize KMC Dictionary in according to DSK_Rel_Ord
            ...
            //FIXME: Transform [Dk Fk] into Dk and Fk separated files
            ...
            // FIXME: Remove Dk, Fk, Gk fragments files from DP3 and DP0 scenario
            ...
            // FIXME: Remove Dk, Fk and Gk fragments files from DP1 scenario
            ...
            //FIXME: create scenario DP2 and put results into DP2_file
            System.out.println("Create DP2 scenario");
            DP_scenario_012(exp_folder, exp, k, pre_proc_DSK_time, 2, b, DEBUG);

            //FIXME: Remove Dk, Fk and Gk fragments files from DP2 scenario
        ...
        // test jar usage: java -jar TradeOff.jar data_directory dataset experiment_directory DEBUG[0/1]
        // java -jar TradeOff.jar /home/user/IdeaProjects/TradeOff/data StaphAU /home/user/IdeaProjects/TradeOff/experiments 1
    }
    ...
    private static void extract_SPSS(String dataset_pathname, int k, String exp_folder, String exp_name, int debug) throws IOException, InterruptedException {
        ...
        if ( (size_D > (size_Block_ESS_Dataset)) && (k > 8) ) {    //We are in Approximate Solution Context
            ...
            // FIXME: Apply ESS to every single Block
            Extract_ESS_SPSS(exp_folder, exp_name, k, num_Blocks, debug);

            // FIXME: concatenate blocks and rename as in the single block case
            ...
            // FIXME: compute ESS on the entire genomic dataset
    ...
    private static void Concatenate_ESS_Blocks(String block, int num_blocks, String ess_output, int debug) {
        ...
        try {
            ...
            if (debug==1) System.out.println("Numbers of ESS Blocks: " + num_blocks);
    ...
    private static void Divide_Dataset_into_Blocks(String dataset_pathname, int k, String exp_folder, String exp_name, int debug) {
        ...
        try {
            ...
            //FIXME: write Block B_residues for recovery k-mer lost.
            ...
            while (((string_D = Dataset_in.readLine()) != null)) {
                ...
                if (bytes_readed < size_Block_ESS_Dataset) {   // If I am in the range of block size allowed
                    ...
                    // start new block
                    //FIXME: writing on Bi-Block in FASTA format
                    Block_i_out = null;
                    try {
                        Block_i_out = new PrintWriter(new FileWriter(exp_folder + FS_SEPARATOR + "k" + k + FS_SEPARATOR + exp_name + "-ESS-k" + k + "_Block_" + count_block + FASTA_EXT));
                    ...
                    if (string_D.startsWith(">")) {
                        ...
                        //FIXME:  instead of storing all the previous read and the next read, I store the last (k-1) nucleotides of last_prev,
    ...
    private static void Remove_Temporary_files(String exp_folder, String exp_name, int k, int dp_scenario, int debug) throws IOException, InterruptedException {
        ...
        // remove path folder (es. /home/user/IdeaProjects/TradeOff/experiments/StaphAU/k4/StaphAU-DSK-k4-frequency/ )
        String path = exp_folder + FS_SEPARATOR + "k" + k + FS_SEPARATOR + exp_name + "-DSK-k" + k + mode;

        remove_folder(path, debug);

        // remove "sorted" DSK output file (es. /home/user/IdeaProjects/TradeOff/experiments/StaphAU/k4/StaphAU-DSK-k4-frequency.txt )
    ...
    private static void Base_Case_SD_RAM(String dataset_pathname, String exp_folder, String exp, String DP_folder, int debug, PrintWriter dpBase_size_out, PrintWriter dpBase_time_out) throws IOException, InterruptedException {
        ...
        if (file.exists() && file.isFile()) {
            ...
            // FIXME: Compressione via FM-index (Build)
            long FM_size_D = 0;
            ...
            // FIXME: Decompressione via FM-index (Extract)
    ...
    private static void Manage_Fa_fragments(String path_in, String exp, int DP_scenario, int approx, PrintWriter DPfile_size, PrintWriter DPfile_time, int gap, int debug) throws IOException, InterruptedException {
        ...
        long BIC_size_fk = 0;
        Lemire_Fk_BIC_Compr_exp.execute(path_in + FS_SEPARATOR, exp + gap_Suffix, scenario_Lemire, approx_Suffix, debug);
        ...
        Lemire_Fk_BIC_Decompr_exp.execute(path_in + FS_SEPARATOR, exp + gap_Suffix, scenario_Lemire, approx_Suffix, debug);
        ...
        Lemire_Fk_OptPFOR_Compr_exp.execute(path_in + FS_SEPARATOR, exp + gap_Suffix, scenario_Lemire, approx_Suffix, debug);
        ...
        Lemire_Fk_OptPFOR_Decompr_exp.execute(path_in + FS_SEPARATOR, exp + gap_Suffix, scenario_Lemire, approx_Suffix, debug);
    ...
    private static void DP_scenario_012(String exp_folder, String exp_name, int k, long DSK_time, int DP_scenario, int b, int debug) throws IOException, InterruptedException {
        //FIXME: DP0, DP1 and DP2 scenario
        ...
        //FIXME: create DSK Dictionary with Gaps before managing Fb_fragments and storing into parent directory (i.e. where is StaphAU-DSK-k4-verbatim.txt)
        ...
        if (DP_scenario == 0) {
            ...
            //FIXME: no longer used because BCSF by construction works with non-negative occurrences.
        }
    ...
    private static void DP_scenario_3(String exp_folder, String exp, int DP_scenario, int k, long pre_proc_dsk_time, long pre_proc_ess_time, int b, int debug) throws IOException, InterruptedException {
        ...
        if (Approx_Solution == 0) { // We are in Exact Solution Context - CD-NRAM
        ...
        } else { // We are in Approximate Solution Context - CD-NRAM    //FIXME: Approx_Solution == 1
            ...
            // FIXME: Read Num_Blocks from .nfo file
            ...
            // FIXME: Compute DSK for every Block od Dataset DSK for Dictionary extraction
            long pre_proc_DSK_start = System.currentTimeMillis();
        ...
        if (Approx_Solution == 0) { // We are in Exact Solution Context - SD-RAM
            ...
            //FIXME: Recovery Dk from S (in .fasta format)
            ...
            //FIXME: prepare Dk for Fk recovering
            ...
            //FIXME: Recovery Fk from Dk on BCSF data structure (in .txt format) without gaps
        ...
        } else {  // We are in Approximate Solution Context - SD-RAM    //FIXME: Approx_Solution == 1
            ...
            //FIXME: Extract from FM-index(S_i)
            long bijection_Extract_from_FMindex_Of_S_start_time = System.currentTimeMillis();
            ...
            //FIXME: Recovery Dk from S_i (in .fasta format)
            long bijection_Recovery_Dk_from_S_start_time = System.currentTimeMillis();
    ...
    private static void Recovery_Fk_from_Dk_and_BCSF(String exp_folder, String exp, int k, int num_blocks, int gap, int debug) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < num_blocks; i++) {
            ...
            //FIXME: distinguish between cases where I have or do not have the associated bloom filter
    ...
    private static void Extract_from_FMindex_Of_S(String exp_folder, String exp, String bijection_suffix, int k, int num_blocks, PrintWriter dPscenarioSD_size_out, int debug) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < num_blocks; i++) {
            ...
            //FIXME: remove temporary decompression FM-index .dec file
